...
4. **息屏后维持 Hey Google 可用** —— Google 在部分 ColorOS 场景会在息屏后停掉软件热词检测，模块会在息屏完成后短延迟恢复已有的 Google hotword session，避免"开机第一次能唤醒，之后息屏唤不醒"的问题。

以上功能均可在模块自带配置界面中独立开关。

## 配置界面

模块 UI 基于 [Miuix](https://github.com/compose-miuix-ui/miuix) 0.9.2，安装后桌面出现 FuckAndes 图标。所有开关默认开启，切换实时生效。
...
GoogleAppHooks.kt          Google 进程内设备伪装与锁屏补语音输入
...
ui/SettingsScreen.kt       Miuix 设置页，6 个功能开关
```
